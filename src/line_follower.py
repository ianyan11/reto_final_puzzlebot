...
class Line_Follower():
    def __init__(self) -> None:
        self.image_sub = rospy.Subscriber('/video_source/raw', Image, self.process_image)
        self.center_line_pub = rospy.Publisher('/line_pose', Int16, queue_size=1)
        self.horizontal_line_pub = rospy.Publisher('/horizontal_line', Empty, queue_size=1)
        self.tolerance = 15000
        #self.mod_image_pub = rospy.Publisher('/mod_video', Image, queue_size=1) 
        #self.twist = Twist()
    ...
    def process_image(self, msg: Image) ->None:
        ...
        self.horizontal_line(gray)
        
        """  t=Twist()
        half =int(len(s)/2)+1
        t.angular.z = -(min_index-half)/half*.35
